...
function ModalSelectTokenNoBalance({
  onClose,
  oppositeAddress,
  setDataAddressToken,
  setDataSymbolToken,
}: Props) {
  const { listAddressToken } = useContext(ContractContext)
        ...
        <div className="myYscroll flex flex-col h-[380px]">
          {tempListAddressToken.length > 0 ? (
            tempListAddressToken.map((item, index) => (
              <>
                <div
                  className="flex flex-row justify-between items-center my-2 p-2 border-2 border-transparent hoverBar cursor-pointer"
                  onClick={async () => {
                    if (
...
export default ModalSelectTokenNoBalance
